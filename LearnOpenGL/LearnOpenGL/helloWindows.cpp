...
int main()
{
    ...
    glGenVertexArrays(2,VAOs);  // 生成对应ID的VAO对象
    glGenBuffers(2,VBOs);   // 生成对应ID的VBO对象
    glBindVertexArray(VAOs[0]); // 绑定VAO对象
    
    glBindBuffer(GL_ARRAY_BUFFER,VBOs[0]);  // 将VBO对象绑定到GL_ARRAY_BUFFER上，任何该目标的缓冲都会配置当前绑定的缓冲（VBO）。
    glBufferData(GL_ARRAY_BUFFER,sizeof(firstTriangle),firstTriangle,GL_STATIC_DRAW); // 将定义的顶点数据复制到缓冲的内存中
    ...
    glBindVertexArray(0);
    glPolygonMode(GL_FRONT_AND_BACK,GL_LINE);   // 设置线条绘制
    ...
    {// Render Loop 渲染循环
        ...
        //glDrawElements(GL_TRIANGLES,6,GL_UNSIGNED_INT,0);   // 绘制图案类型，顶点数，索引类型，偏移量
        glBindVertexArray(VAOs[1]);
        glDrawArrays(GL_TRIANGLES,0,3);
